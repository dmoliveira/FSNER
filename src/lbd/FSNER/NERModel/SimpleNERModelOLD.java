...
import lbd.FSNER.Filter.FtrAffix.AffixType;
...
import lbd.FSNER.Filter.old.FtrSingleTermDictionary;
...
public class SimpleNERModelOLD extends AbstractNERModel {

	public SimpleNERModelOLD(DataCollection pDataCollection) {
	...
	protected void allocModelSub(String[] initializeFilenameList) {
		...
		for (int i = 0; i < dataPreprocessorList.size(); i++) {
			...
			/** Test **/
			if(mFilterParameters.isFilterActive(FilterType.Test)) {
				List<AbstractFilter> vFilterList = new ArrayList<AbstractFilter>();
				vFilterList.add(new FtrAffix(i, new FSCMNoScore(), AffixType.Prefix, 2));
				mActivityControl.addActivity(new FtrWindow(i, new FSCMNoScore(), 2, WindowType.Both, vFilterList));
			}
	...
	protected void addEntityProbability(int i) {
		mActivityControl.addActivity(new FtrToken(i));
		// activityControl.getFilterList().get(activityControl.getFilterList().size()-1).setFilterState(FilterState.Auxiliary);
	...
	protected void addDictionaryFiltersOld(int i) {
		...
		for (int j = 0; j < dictionaryListNumber; j++) {
			...
			dictionaryFilters.add(new FtrSingleTermDictionary4(i,
					...
					(j <= 0) ? null
							: (FtrSingleTermDictionary4) dictionaryFilters
							.get(j - 1)));
		}
	...
	@DefaultValue(value="flexibilityIndex = Flexibility.values().length - 3; windowSize = 3; windowSize > 2" +
			"contextTypeIndex = 1; flexibilityIndex > 1 || windowSize > 2")
	protected void addContextFilters(int i) {
		for (int flexibilityIndex = Flexibility.values().length - 3; flexibilityIndex >= 0; flexibilityIndex--) {
			for (int windowSize = 2; windowSize > 1; windowSize--) {
	...
	protected void addStateFilter(int i) {
		...
		int [] vNumberPreviousState = {1,2,3};
		boolean [] vIsToConsiderStatePerTerm = {true};
