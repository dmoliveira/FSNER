...
public class Context implements Serializable{
	...
	public void addAsContext(SequenceLabel sequenceLabel, int index) {
		
		String prefixValue = Symbol.EMPTY;
		String suffixValue = Symbol.EMPTY;
		String mainTerm = sequenceLabel.getTerm(index);
		...
		for(int i = 1; i <= maxWindowSize; i++) {
			...
			if(prefixIndex > -1 && !sequenceLabel.getTerm(prefixIndex).isEmpty()) {
				prefixValue = sequenceLabel.getTerm(prefixIndex) + DELIMITER_TERM + prefixValue;
				prefixSize++;
			...
			if(suffixIndex < sequenceLabel.size() && !sequenceLabel.getTerm(suffixIndex).isEmpty()) {
				suffixValue = suffixValue + DELIMITER_TERM + sequenceLabel.getTerm(suffixIndex);
				suffixSize++;
	...
	public int getContextId(SequenceLabel sequenceLabel, int index, ContextType contextType, 
	...
	protected String generateContext(SequenceLabel sequenceLabel, int index, ContextType contextType, int windowSize) {
		...
		for(int i = 1; i <= windowSize; i++) {
			...
			if(contextType != ContextType.Suffix && prefixIndex > -1 && !sequenceLabel.getTerm(prefixIndex).isEmpty()) {
				prefixValue = sequenceLabel.getTerm(prefixIndex) + DELIMITER_TERM + prefixValue;
				prefixSize++;
			}
			
			if(contextType != ContextType.Prefix && suffixIndex < sequenceLabel.size() && !sequenceLabel.getTerm(suffixIndex).isEmpty()) {
				suffixValue = suffixValue + DELIMITER_TERM + sequenceLabel.getTerm(suffixIndex);
				suffixSize++;
			}
		}
		...
		if((contextType == ContextType.Prefix && prefixSize < windowSize) ||
				...
				(contextType == ContextType.AllContext && (prefixSize < windowSize || suffixSize < windowSize))) contextValue = Symbol.EMPTY;
