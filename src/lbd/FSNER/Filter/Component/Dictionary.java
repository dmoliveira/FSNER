...
public class Dictionary {
	...
	public boolean isTermPartFromEntityEntryInDictionary(ISequence sequenceLabelProcessed, int index) {
		return(getDictionaryEntry(sequenceLabelProcessed, index) != null);
	}

	public DictionaryEntry getDictionaryEntry(ISequence sequenceLabelProcessed, int index) {

		DictionaryEntry dictionaryEntry = null;
		ArrayList<DictionaryEntry> dictionaryEntryList = dictionaryMap.get(sequenceLabelProcessed.getToken(index));
	...
	protected String [] generateCandidateEntryValue(DictionaryEntry dictionaryEntry, ISequence sequenceLabelProcessed, int index) {
		...
		for(int i = startOffSet; i >= 0 && i < sequenceLabelProcessed.length() && i < endOffSet; i++) {
			candiadeEntryValue[i - startOffSet] = sequenceLabelProcessed.getToken(i);
		}
