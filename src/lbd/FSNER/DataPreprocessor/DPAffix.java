...
public class DPAffix extends AbstractDataPreprocessor{
	...
	public DPAffix(int startAffix, int endAffix) {
		...
		if(startAffix > endAffix) new Throwable("DPAffix: StartAffix > EndAffix (" + startAffix + ">" + endAffix + ")");
	}
	...
	public SequenceLabelElement preprocessingTerm(String term, int label) {
		...
		String processedTerm = (endAffix < term.length() && term.length() > MIN_TERM_LENGHT)?
