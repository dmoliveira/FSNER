...
public class DPVowelConsonantCode extends AbstractDataPreprocessor{
	...
	public SequenceLabelElement preprocessingTerm(String term, int label) {
		...
		for(int i = 0; i < term.length(); i++) {
			...
			candidateBlockSize = (term.length()-i > blockSize)? blockSize : term.length()-i;
			processedTerm += convertToCode(term.substring(i, i+candidateBlockSize)) + Symbol.DOT;
			//System.out.println("Bs:" + candidateBlockSize + " " + term.substring(i, i+candidateBlockSize) + " " + processedTerm);
			
			i += candidateBlockSize-1;
		}
