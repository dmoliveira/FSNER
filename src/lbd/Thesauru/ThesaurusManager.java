	...
	public Thesaurus getThesaurusLite(String term) {
		...
		if(!thesaurusMap.containsKey(term)) {
			...
			try {
				...
				if(isDebugActive) {
					System.out.println("-- Elipsed Time: " + ((new Date()).getTime() - startTime.getTime())/1000.0 + "s intern(" + ((new Date()).getTime() - internDate.getTime())/1000.0  + "s)");
				}
			...
			try {
				...
				while((line = queryConnection.readLine()) != null && (!hasThesaurus || !line.equals(TAG_ANALYSIS_END))) {
					...
					if(line.equals(TAG_MAIN_ENTRY)) {
						...
						do { line = removeHTMLTags(queryConnection.readLine()); } while(line.isEmpty());
						if(isDebugActive) {
							System.out.print("Synonyns("+posTag+"):");
						}

						do {
							element = line.split(DELIMITER_ELEMENTS);
							for(int i = 0; i < element.length; i++) {
								tElement.addSynonym(posTag, element[i].replace(TAG_COMMA, TAG_EMPTY));
							}
						...
						do { line = removeHTMLTags(queryConnection.readLine()); } while(line.isEmpty());
						if(isDebugActive) {
							System.out.print("Antonyns("+posTag+"):");
						}

						do {
							element = line.split(DELIMITER_ELEMENTS);
							for(int i = 0; i < element.length; i++) {
								tElement.addAntonym(posTag, element[i].replace(TAG_COMMA, TAG_EMPTY));
							}
