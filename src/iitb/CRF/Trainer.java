...
public class Trainer {
    ...
    public Trainer(CrfParams p, Writer out) {
        params = p; 
        logOut = out;
    }
    ...
    public void train(CRF model, DataIter data, double[] l, Evaluator eval) {
        ...
        if (params.debugLvl > 0) {
            ...
            try {
            	if(logOut != null)
            		logOut.write("Number of features :" + lambda.length + "\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
        }
    ...
    void doTrain() {
        ...
        do {
            ...
            try	{
            	...
            	//@DMZDebug
            	if(logOut != null)
					try {logOut.write("CRF: lbfgs failed.\n"+e + "\n");} catch (IOException e1) {e1.printStackTrace();}
                ...
                if (e.iflag == -1) {
                 ...
                 //@DMZDebug
	             if(logOut != null)
					try {logOut.write("Possible reasons could be: \n \t 1. Bug in the feature generation or data handling code\n\t 2. Not enough features to make observed feature value==expected value\n\n");} catch (IOException e1) {e1.printStackTrace();}
                }
                return;
            }
            icall += 1;
    ...
    protected double computeFunctionGradient(double lambda[], double grad[]) {
        ...
        try {
            ...
            //-- @DMZDebug
            if (params.debugLvl > 0); {
            	String debugOutput = "Iter " + icall + " log likelihood "+logli + " norm(grad logli) " + norm(grad) + " norm(x) "+ norm(lambda);
            	debugOutput += " (" + (int)(((new Date()).getTime() - trainingStartTime.getTime())/1000) + "s)";
                //Util.printDbg(debugOutput);
                if(logOut != null) logOut.write(debugOutput + "\n");
            }
            if (icall == 0) {
                System.out.println("Number of training records " + numRecord);
                if(logOut != null) logOut.write("Number of training records " + numRecord + "\n");
            }
        } catch (Exception e) {
            System.out.println("Alpha-i " + alpha_Y.toString());
            System.out.println("Ri " + Ri_Y.toString());
